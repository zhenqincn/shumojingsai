...
if __name__ == '__main__':
    ...
    city_pair_population_tuple_list = sorted(city_pair_population_tuple_list, key=lambda item: item[2], reverse=True)
    ...
    while len(cur_road_info_list) < num_links:
        if_add_one_link = False  # 如果一次循环中没有加入任何的连接，说明再也不能加边，应该终止循环
        print(len(cur_road_info_list))
        for city_tuple_list in city_pair_population_tuple_list:
            # 如果加入了一条边，就应该终止当前的循环，并且在城市对中删掉这一对城市
            loop_flag = True  # for 的循环标志位
            # 获得当前城市对的两个城市编号，以及两个城市的联合人口权重
            tmp_a, tmp_b, tmp_population = city_tuple_list[0], city_tuple_list[1], city_tuple_list[2]
            # 如果当前城市对中两个城市已经直接连接，那么不需要再考虑中继节点
            if adj_matrix[tmp_a][tmp_b] != 1:
                ...
                for middle in range(12):
                    ...
                    if adj_matrix[tmp_a][middle] != 1 and adj_matrix[tmp_b][middle] == 1:
                        ...
                        if convert_distance_to_level(distance_matrix[tmp_a][middle]) > cur_distance_level and convert_distance_to_level(distance_matrix[tmp_b][middle]) > cur_distance_level:
                            ...
                            print("加入", city_list[road.start], '-', city_list[road.end], '这条连接')
        ...
        if not if_add_one_link:
            break
    print('目前地图中一共有', len(cur_road_info_list), '条边')
